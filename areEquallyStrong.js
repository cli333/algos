...
function solve(yL, yR, fL, fR) {
  const yStrongest = yL > yR ? yL : yR;
  const yWeakest = yStrongest === yL ? yR : yL;
  const fStrongest = fL > fR ? fL : fR;
  const fWeakest = fStrongest === fL ? fR : fL;

  return yStrongest === fStrongest && yWeakest === fWeakest;
}

console.log(solve(15, 10, 15, 9));
console.log(solve(15, 10, 15, 10));
